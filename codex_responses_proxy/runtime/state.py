...
def sanitization_count(note: str, field: str) -> int:
    ...
    _, found, tail = note.partition(f"{field}=")
    with suppress(ValueError):
        return max(0, int(tail.split(" ", 1)[0])) if found else 0
    return 0
...
def remember_empty_response_failure(
    ...
    *,
    ...
    with _EMPTY_RESPONSE_FAILURES_LOCK:
        ...
        oldest = sorted(_EMPTY_RESPONSE_FAILURES, key=_EMPTY_RESPONSE_FAILURES.__getitem__)
        for oldest_key in oldest[:-capacity]:
