...
selected upstream. Unknown or structurally unproved replay material fails locally.
...
Encrypted delegation additionally requires an upstream able to interpret that
native control payload; cross-provider decryption is not guaranteed. A visible
message envelope is not proof that its task body survived transport.
...
claim excludes encrypted native delegation. Provider-specific recovery consumes the same
