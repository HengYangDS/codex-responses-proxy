...
PACKAGE = ROOT / "codex_dmx_proxy"
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from codex_dmx_proxy.listener import control, identity, responses, server, sse  # noqa: E402
from codex_dmx_proxy.release import digest, inventory  # noqa: E402
from codex_dmx_proxy.supervision import select as service_selection  # noqa: E402
...
class ProxyOwnerBoundaryContracts(unittest.TestCase):
    ...
    def test_owner_modules_never_import_the_entrypoint(self) -> None:
        for relative in ("control.py", "server.py", "responses.py", "sse.py"):
            tree = ast.parse((PACKAGE / "listener" / relative).read_text())
            imports = {
                alias.name
            ...
            self.assertNotIn("codex_dmx_proxy.listener.entrypoint", imports, relative)
    ...
    def test_loaded_identity_accepts_only_exact_loaded_release(self) -> None:  # noqa: C901
        with tempfile.TemporaryDirectory() as directory:
            ...
            files = {"VERSION": digest.sha256_file(root / "VERSION")}
