...
- 将上游或网关返回的 HTTP `524`（超时）归入既有的瞬态上游失败分类，与 `429`、`500`、`502`、`503`、`504` 一样使用受限的指数退避重试预算；不改变不可重试的 schema 或加密内容错误处理。

### 验证

- 新增 HTTP `524` 分类回归；Python 3.12、3.13、3.14 继续执行同一套跨平台包与数据面测试。

## [1.0.5] - 2026-07-14

### 修复
- **原会话恢复边界正式化**：安装与卸载不再要求强制退出/重开 Codex 或新建会话；它们只说明已运行客户端须按自身正常生命周期重载配置，并明确原会话仍须在原会话内获得用户可见的成功回复后才能宣称恢复。
- **控制面单一权威**：检测到 AIGW 标记的 provider projection 时，安装器保留该投影不直接改写；代理仍只管理可再生 payload 与本地 listener/watchdog 生命周期，避免与 AIGW 的 endpoint 和三投影同步发生写入竞争。

### 验证
- Python 3.12、3.13、3.14：各 47 项回归通过；覆盖 AIGW-owned route 不被安装器改写，以及生命周期脚本不再要求重开客户端/新建会话并保留原会话边界。

## [1.0.4] - 2026-07-14

### 增强
- **运行时 payload 可验证**：为已部署的可执行文件写入仅含版本与 SHA-256 的清单；清单不覆盖配置、备份、日志、Token、请求体或会话内容。
- **精确重载**：`control.py reload` 先校验清单，再仅终止端口与命令行均验证属于本安装目录的 Proxy，并确认 watchdog 拉起不同 PID 的替代监听进程。
- **权威边界收敛**：AIGW 管理的 provider projection 不由 Proxy 改写；安装、控制与卸载均保留 AIGW 的 endpoint/configuration 权威。

### 验证
- Python 3.12、3.13、3.14：各 46 项测试通过；新增部署清单防篡改、旧运行时残留清理、AIGW 路由边界与精确重载回归。
