...
def compile_sources(root: Path = ROOT) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="codex-responses-proxy-pycache-") as cache:
...
def abnormal_output(stdout: bytes, stderr: bytes) -> bool:
    """Return whether a passing test leaked an exception, warning, or error banner."""
