...
Nox SHALL own the verification graph and the committed uv lock SHALL own its
Python tool environment. Local execution, GitLab CI, and GitHub Actions SHALL
invoke that graph rather than duplicate tool versions, interpreter matrices,
coverage policy, or test sequences.
