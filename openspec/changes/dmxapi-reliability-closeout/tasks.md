## 1. Contract and root cause

- [x] 1.1 Observe the released runtime and separate current-process events from
  predecessor-process history.
- [x] 1.2 Bind the exact live reason `empty_text_content` without retaining
  request content.
- [x] 1.3 Define the narrow owner, negative boundaries, rollback, and
  multi-plane completion contract.
- [x] 1.4 Bind the Change to the active claim and pass strict OpenSpec
  validation.

## 2. TDD implementation

- [x] 2.1 Prove focused RED behavior for paired empty function/custom-tool
  outputs while empty dialogue remains rejected.
- [x] 2.2 Implement only the paired-output marker in the output projector.
