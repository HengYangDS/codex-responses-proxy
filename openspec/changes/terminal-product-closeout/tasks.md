## 1. Freeze and reconcile current truth

- [x] 1.1 Freeze the terminal lane head, index, unstaged patch, untracked paths,
...
- [x] 4.1 Prove `store=false`, provider item IDs, encrypted replay,
  `compaction_trigger`, tool pairing, supported non-text agent content,
  malformed input, and unknown structure behavior without client-state access.
- [x] 4.2 Prove DMXAPI classified empty-response exact retry and 503 exhaustion,
  UCloud non-text recovery, and response-integrity behavior.
- [x] 4.3 Prove HTTP 429 uses one upstream call, preserves eligible response
  data, enforces bounded monotonic provider cooldown, isolates providers, and
  keeps ordinary-request concurrency outside the proxy while retaining the
  lifecycle drain barrier and active-request accounting.
- [x] 4.4 Prove the DMXAPI, UCloud, and AIHubMix switch-and-return matrix
  through identical portable conversation fixtures.
