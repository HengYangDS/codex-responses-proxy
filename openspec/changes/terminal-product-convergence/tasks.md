...
- [ ] 4.3 Add adversarial contracts for malformed containers, unknown item types, encrypted content, tool-call pairing, provider-local items, streaming control events, non-stream terminal responses, and structured Provider errors. The isolated 3.1.17 patch candidate preserves inline image data URLs through projection and exact retries. Its source, native package, publication and installed visual acceptance are tracked separately; the full terminal convergence Change remains incomplete.
