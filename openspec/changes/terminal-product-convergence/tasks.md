...
- [ ] 4.3 Add adversarial contracts for malformed containers, unknown item types, encrypted content, tool-call pairing, provider-local items, streaming control events, non-stream terminal responses, and structured Provider errors. Inline PNG/JPEG/WebP/GIF image data URLs now retain their exact transport bytes, detail and ordering through the shared projection. Image-only system/developer/user messages and paired function/custom-tool results survive repeated projection; classified DMX retries preserve the exact outbound image bytes. The repository regression reproduces 21 failing preservation cases before the correction, then passes all 31 content cases; 167 protocol and 120 relay/provider/reliability tests pass, with 174 subtests, and the two wire retry variants pass separately. An independent handoff probe passes 28/28. Ruff and scoped typing pass. Evidence is under `build/verification/07b73a221b0f138527aac022ecfc8ccfc1208cbc/inline-image-repair/`. The same image semantics also shipped in accepted 3.1.17, now incorporated without restoring retired module paths or duplicating tests. Current-candidate release, installed-runtime and model-visible pixel acceptance remain unproven; no running service or session history was changed.
...
- [ ] 4.7 Verify request and response performance, bounded retries, cooldown monotonicity, cancellation, connection reuse, memory, and streaming latency against explicit budgets without weakening correctness or multiplying retries. SSE recovery now classifies decoded event data instead of JSON byte spellings: control frames retain pre-content admission; only output-free `server_error` failures reuse the existing bounded reconnect budget. Committed output, tool calls, permanent errors, and unknown failures are preserved without replay; reconnect delays cannot exceed the remaining stream budget. Error class and bounded request UUID diagnostics exclude upstream prose. The built wheel passes 303 tests and 174 subtests on both Python 3.12 and 3.14.7; a frozen macOS listener passes a real loopback failed-then-completed stream with identical upstream request bytes, one visible result, exact teardown, and unchanged host service inventory. Evidence: `build/verification/43e62b9a3df5072cd28a74b417123f789d45de0b/stream-recovery/`. This is candidate evidence only: production 3.1.17 remains unchanged; release integration, native Windows/Linux qualification, and original-thread recovery remain unproven.
...
- [ ] 10.1 Determine the next version from actual public compatibility, update sole-owner `VERSION`, package metadata, Changelog, documentation, and asset naming, and verify strict SemVer consistency. The prepared successor is 4.0.0 because commit `17750153` removes the public flat-installation migration path; internal package movement alone is not the reason for the major version. `VERSION` remains the sole package-version source, the Changelog explains the removal, metadata admission and 116 release/artifact tests pass. This is an unpublished candidate, not an installed or released version.
