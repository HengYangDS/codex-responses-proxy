#!/bin/sh
# Project one canonical accepted ref into a GitHub identity history. Only a
# newly created isolated clone is rewritten; provider-native tags are immutable.
set -eu

usage() {
  cat >&2 <<'USAGE'
usage: project-github-forge.sh [--source-ref <ref>] [--github-remote <name>]

Projects a clean canonical ref into the GitHub peer with the GitHub commit
...
canonical=$(git rev-parse --verify --end-of-options "$source_ref^{commit}")
...
python3 "$root/scripts/rewrite-provider-history.py" \
  ...
  --target-ref "refs/heads/$target_branch" \
  --name "$github_name" \
  --email "$github_email" \
...
projected=$(git -C "$projection" rev-parse "refs/heads/$target_branch")
[ "$(git -C "$projection" rev-parse "$projected^{tree}")" = "$canonical_tree" ] || { echo "projected GitHub branch tree differs from canonical source" >&2; exit 1; }
...
remote_tip=$(git_transport -C "$projection" ls-remote --heads github "refs/heads/$target_branch" | awk 'NR==1 {print $1}')
if [ -n "$remote_tip" ]; then
  git_transport -C "$projection" fetch --quiet --no-tags github "refs/heads/$target_branch:refs/remotes/github/$target_branch"
  remote_tree=$(git -C "$projection" rev-parse "refs/remotes/github/$target_branch^{tree}")
  if ! git -C "$projection" log "$canonical" --format='%T' | grep -F -x "$remote_tree" >/dev/null; then
    echo "GitHub main tree diverges from canonical history; resolve manually" >&2
    exit 1
  fi
fi
...
lease="refs/heads/$target_branch:${remote_tip:-0000000000000000000000000000000000000000}"
git_transport -C "$projection" -c user.name="$github_name" -c user.email="$github_email" -c user.useConfigOnly=true \
  push --force-with-lease="$lease" github "refs/heads/$target_branch:refs/heads/$target_branch"
