...
- **代理**（`proxy/dmx_responses_proxy.py`）：透传 method/path/headers（含 Bearer token）；
  ...
  `input_image`（本地路径与 Data URL），保留相邻文本和合法 `http(s)` 图片。**fail-open**：任何解析错误
  → 原样转发。
...
图片路径，第三方 Responses 端点要求远程 URL，因而会拒绝整个后续请求。代理会仅剔除这类不可远程回放的
`input_image`（也包括该端点不支持的 Data URL），其余文字和合法 `http(s)` 图片保持不变；无需改写会话历史。
