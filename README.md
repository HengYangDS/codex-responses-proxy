...
| Legitimate long turns keep exhausting that queue wait | `python3 -m codex_responses_proxy.commands.control status --json` | Raise `CODEX_RESPONSES_PROXY_RESPONSES_QUEUE_TIMEOUT`; a waiting request holds no process-wide slot, because the route slot is acquired first. Under native supervision the installed unit pins the install-time value, so exporting the variable in a shell does not reach the listener — reinstall with `--force-v2-bootstrap` to re-render the unit. |
