...
:mod:`codex_dmx_proxy.listener.rewrite`; request admission and upstream
orchestration remain in :mod:`codex_dmx_proxy.listener.responses`.
"""
...
def _read_one_stream(
    ...
    def process_event(event: bytes) -> None:
        ...
        sanitized, removed = rewrite.sanitize_sse_event(event)
        if removed:
            stripped_events += 1
