...
from codex_responses_proxy.lifecycle import owned_files, state
...
_RETIRED_RUNTIME_FILES = {
    1: frozenset(
        {
            "VERSION",
            "codex_responses_proxy/lifecycle/control.py",
            "governance.py",
            ...
            "codex_responses_proxy/lifecycle/supervision/watchdog.py",
        }
    ),
    2: frozenset(
        {
...
_PREDECESSOR_RECEIPT_FILENAME = "release-source-receipt.json"
_PREDECESSOR_PATHS = {
    inventory.EXECUTABLE: "codex_responses_proxy/listener/entrypoint.py",
    inventory.PROVIDER_MANIFEST: "codex_responses_proxy/providers/manifest.toml",
}
_DIRECT_PREDECESSOR_INVENTORY = (
    frozenset(
        {
            "VERSION",
            ...
            "codex_responses_proxy/payload/artifact.py",
...
_HISTORICAL_RUNTIME_INVENTORIES = (
    *_RETIRED_RUNTIME_FILES.values(),
    _DIRECT_PREDECESSOR_INVENTORY[0],
)
RETIRED_OWNED_FILES = frozenset().union(*_HISTORICAL_RUNTIME_INVENTORIES)
RETIRED_OWNED_FILES = RETIRED_OWNED_FILES.union(
    {_PREDECESSOR_RECEIPT_FILENAME, inventory.INSTALLED_RELEASE_STATE_FILENAME}
)
...
def purge_installed_projection(ctx: runtime_context.RuntimeContext) -> tuple[str, ...]:
    ...
    current = (
        ...
        and set(files)
        in (set(inventory.runtime_files()), set(inventory.runtime_files(windows=True)))
    )
    if current:
        ok, detail = verify_payload_manifest(ctx)
        if not ok:
            raise errors.InstallError(f"installed payload integrity check failed: {detail}")
        owned = {str(relative) for relative in files} | set(owned_files.OWNED_PAYLOAD_METADATA)
    else:
...
def _payload_relative_paths(root: str) -> list[str]:
    ...
    missing = [relative for relative in files if not os.path.isfile(os.path.join(root, relative))]
    if missing:
    ...
    entrypoint = next(
        (
            ...
            for relative in (
                ...
                "codex_responses_proxy/listener/entrypoint.py",
                "codex_responses_proxy/service/entrypoint.py",
                "proxy/dmx_responses_proxy.py",
            )
...
def _historical_manifest_files(
    ...
    predecessor = (
        _DIRECT_PREDECESSOR_INVENTORY
        if schema == 2 and set(files) == set(_DIRECT_PREDECESSOR_INVENTORY[0])
        else None
    )
    expected_files = predecessor[0] if predecessor is not None else None
    ...
    if predecessor is not None:
        metadata.add(_PREDECESSOR_RECEIPT_FILENAME)
...
def _verify_protocol_v2_metadata(
    ...
    receipt_path = owned_files.regular_file(
        install, _PREDECESSOR_RECEIPT_FILENAME, "historical release receipt"
    )
